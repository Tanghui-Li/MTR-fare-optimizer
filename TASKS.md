...
- Bottom Sheet 标题栏已支持向上拖展开、向下拖收起，保留点击展开/收起。

建议移动端结构：

- 地图作为主画布。
- 底部使用可拖拽 Bottom Sheet；当前已覆盖基础拖拽手势，后续可接入 Vaul 获得连续跟手动画和更完整的无障碍抽屉语义。
...
- 左侧控制/结果面板。
- 右侧地图。
- 左侧宽度已收敛到约 `460px-520px`，避免桌面端地图被过度压缩。
...
- 移动端图层开关已收进“图层”按钮，减少地图顶部常驻遮挡。

建议：

- 默认突出 MTR 路线规划主任务。
- 桌面端继续展示图层开关，移动端通过“图层”按钮按需展开。
- 图例移动端默认折叠。
...
- 已加入 Vitest 和 `npm run test`。
- 已补 `src/pathfinder.test.ts`，覆盖同站和多跳最低价路径。
- 已补 `src/routePlanner.test.ts`，覆盖同站、真实票价矩阵主路径和不可达路径。
...
- 继续补充更具体的票价回归用例，例如跨线换乘、AEL 免费接驳、轻铁/港铁巴士组合路径。
...
- 默认关闭的轻铁、巴士站、实时巴士地图图层已改为 React lazy 按需加载，减少首屏主路径需要初始化的模块。

建议：

- 数据恢复后重新构建确认真实体积。
- 后续可继续把无障碍筛选或弹窗详情拆成按需模块，但需要先确认交互收益大于加载延迟。
